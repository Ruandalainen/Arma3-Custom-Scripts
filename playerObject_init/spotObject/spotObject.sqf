...
this addAction [
	"Spot", 
	{
		...
		if (str _side == "UNKNOWN") then {
			...
			if (str _type == "<NULL-Object>") then {
				_0 = 'Static object';
				_params set [0, _0];
				_type = _params select 0;	
				titleText [_type + ", Distance: " + str _dist, "PLAIN DOWN", 0.3]; // YOU CAN TAKE THE DISTANCE OFF THE "titleText" BY REMOVING "", Distance: " + str _dist +"
			} 
		}
		else {
			_params = getCursorObjectParams;
